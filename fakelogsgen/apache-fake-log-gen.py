...
parser.add_argument("--date", "-d", dest='arg_date', help="Date format YYYYMMDD e.g., 20210225", type=str)
...
in_date = args.arg_date

faker = Faker()

#timestr = time.strftime("%Y%m%d-%H%M%S")
timestr = time.strftime("%Y%m%d") if not in_date else datetime.datetime.strptime(in_date, '%Y%m%d')
in_datetime = in_date + ' 00:00:00'
otime = datetime.datetime.now() if not in_date else datetime.datetime.strptime(in_datetime, '%Y%m%d %H:%M:%S')

outFileName = 'access_log_'+in_date+'.log' if not file_prefix else file_prefix+'_access_log_'+in_date+'.log'
dirname = os.path.dirname(__file__)

for case in switch(output_type):
    if case('LOG'):
        # f = open(outFileName,'w')
        f = open(os.path.join(dirname+'/logs/', outFileName),'w')
        break
    if case('GZ'):
